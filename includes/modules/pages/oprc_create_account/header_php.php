...
if (isset($_POST['oprcaction']) && ($_POST['oprcaction'] == 'process')) {
    ...
    if (!isset($_SESSION['customer_id']) || !$_SESSION['customer_id']) {
        ...
        if (OPRC_RECAPTCHA_STATUS == 'true') {
            if ($captcha == '') {
                $error = true;
                $messageStack->add_session('email_address_register', 'Please check the Google reCAPTCHA box');
                echo "<script>function reload() { location.reload(); window.alert('Please check the Google reCAPTCHA box'); } reload();</script>";
            } else {
