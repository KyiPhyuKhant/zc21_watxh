...
<?php if ((int)FEC_MESSAGESTACK_DURATION > 0) { ?>
  setTimeout("jQuery('div#messageStackErrors').fadeOut()", <?php echo (int)FEC_MESSAGESTACK_DURATION; ?>); // ignore syntax error
<?php } ?> 
}
function blockPage() {
  jQuery.blockUI({
    message: '<?php echo addslashes(FEC_PROCESSING_TEXT); ?>', 
    css: { 
      border: 'none', 
      padding: '15px', 
      backgroundColor: '<?php echo FEC_MESSAGE_BACKGROUND_COLOR; ?>', 
      '-webkit-border-radius': '10px', 
      '-moz-border-radius': '10px', 
      opacity: '<?php echo FEC_MESSAGE_OPACITY; ?>', 
      color: '<?php echo FEC_MESSAGE_TEXT_COLOR; ?>' 
    },
    overlayCSS: { 
      backgroundColor: '<?php echo FEC_MESSAGE_OVERLAY_COLOR; ?>',
      color: '<?php echo FEC_MESSAGE_OVERLAY_TEXT_COLOR; ?>',
      opacity: '<?php echo FEC_MESSAGE_OVERLAY_OPACITY; ?>'
    } 
  });
}
--></script>