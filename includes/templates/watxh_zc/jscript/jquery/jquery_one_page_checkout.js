...
jQuery(document).ready(function()
  {
    ...
    if (oprcRemoveCheckout != '') {
      ...
      jQuery(document).on('click', 'a.removeProduct', function()
        {
          blockPage(true, true);
          if (jQuery('a.removeProduct').length == 1) {
            return true;
          } else {
  ...
  */
                ...
                jQuery.get(ajaxLoginCheckURL, function(loginCheck)
                  {
                    if (parseInt(loginCheck) == 1) {
                      ...
                      var scripts = jQuery("#onePageCheckout").find("script");
                      scripts.each(function()
                        {
                          ...
                          if (this.src) {
                            ...
                            newScript.textContent = this.textContent || this.innerText;
                          }
                          document.body.appendChild(newScript);
                        }
                      );
                      document.dispatchEvent(new Event('onePageCheckoutReloaded'));
                      reconfigureSideMenu();
                      displayCreditUpdate();
                      scrollToAddresses();
                      ...
                      setTimeout(function()
                        {
                          location.reload(true); // true to force reload from the server and not the cache
                        }, 1000
    ...
    jQuery(document).on('click', '#js-submit input[type="submit"]', function(e)
      {
        checkAddress();
    ...
    jQuery(document).on('click', '#shopBagWrapper .couponRemove', function()
      {
            ...
            jQuery.get(ajaxLoginCheckURL, function(loginCheck)
              {
                if (parseInt(loginCheck) == 1) {
                  if (window.location.hash == '#checkoutShipAddressDefault' || window.location.hash == '#checkoutPayAddressDefault') {
                    ...
                    if (response && typeof response.oprcAddresses !== 'undefined') {
                      jQuery('#oprcAddresses').html(response.oprcAddresses);
                    }

                    if (response && typeof response.shippingMethodContainer !== 'undefined') {
                      jQuery('#shippingMethodContainer').html(response.shippingMethodContainer ? response.shippingMethodContainer : '');
                    }

                    if (oprcRefreshPayment == 'true' && response && typeof response.paymentMethodContainer !== 'undefined') {
                      jQuery('#paymentMethodContainer').html(response.paymentMethodContainer);
                    }

                    if (response && typeof response.shopBagWrapper !== 'undefined') {
                      jQuery('#shopBagWrapper').html(response.shopBagWrapper);
                    }
...
function reconfigureSideMenu() {
  ...
  if ((!oprcBrowserInfo.isIE || oprcBrowserInfo.ieVersion > 7)) {
    setStickColumnWidth();
    stickySideMenu();
  }
...
function stickySideMenu() {
  ...
  //if(jQuery(window).width() >= 1000 && (!oprcBrowserInfo.isIE || oprcBrowserInfo.ieVersion > 7)) {
  if(!isTouchDevice() && (!oprcBrowserInfo.isIE || oprcBrowserInfo.ieVersion > 7)) {
  ...
  jQuery.get(ajaxLoginCheckURL, function(loginCheck)
    {
      if (parseInt(loginCheck) == 1) {
        jQuery.post(onePageCheckoutConfirmURL, jQuery("[name=checkout_payment]").serialize() + '&request=ajax', function(data)
          {
            var redirectPayload = null;
            if (typeof data === 'object' && data !== null) {
              redirectPayload = data;
            } else if (typeof data === 'string') {
              var trimmedData = data.trim();
              if (trimmedData.charAt(0) === '{') {
                try {
                  redirectPayload = JSON.parse(trimmedData);
                } catch (e) {
            ...
            if (redirectPayload && redirectPayload.redirect_url) {
              jQuery.unblockUI();
              window.location.assign(redirectPayload.redirect_url);
              return;
            }

            var responseHtml = (typeof data === 'string') ? data : '';
            var $response = jQuery(responseHtml);
            var onePageCheckoutContent = $response.find('#onePageCheckoutContent').html();
            ...
            if (!messagesExist) {
              var $confirmationForm = jQuery('[name="checkout_confirmation"]');
              if ($confirmationForm.length === 0) {
                jQuery.unblockUI();
                return false;
              }

              var $submitButton = $confirmationForm.find('input[type="submit"], button[type="submit"]').first();
              var submitEvent = jQuery.Event('submit');
              $confirmationForm.trigger(submitEvent);
              if (submitEvent.isDefaultPrevented()) {
                if ($submitButton.length > 0) {
                  $submitButton.prop('disabled', false).css('cursor', '');
                }
                jQuery.unblockUI();
                return false;
              }
              ...
              jQuery(document).trigger('oprc:checkoutConfirmationLoaded', [$confirmationForm]);

              var formAction = $confirmationForm.attr('action') || '';
              if (formAction.indexOf('checkout_process') === -1) {
                blockPage(true, true);
              ...
              blockPage(true, true);
              $submitButton = $confirmationForm.find('input[type="submit"], button[type="submit"]').first();
              if ($submitButton.length > 0) {
                $submitButton.prop('disabled', true).css('cursor', 'not-allowed');
              }

              // Allow payment modules to intercept the checkout-process submission.
              var beforeProcessEvent = jQuery.Event('oprc:beforeCheckoutProcess');
              jQuery(document).trigger(beforeProcessEvent, [$confirmationForm]);
              ...
              jQuery.post(ajaxCheckoutProcessURL, $confirmationForm.serialize(), null, 'json')
                .done(function(response) {
                  ...
                  if (response && response.messages) {
                    if (typeof displayErrors === 'function') {
                      ...
                      jQuery('.messageStackErrors').html(response.messages);
                    }
                  }
