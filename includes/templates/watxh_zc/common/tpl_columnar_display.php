...
<div class="row product-item-listing">
    ...
    if (isset($_GET['cPath'])) {
        $cPath_parts    = explode('_', $_GET['cPath']);
        $current_cat_id = (int) end($cPath_parts);
    } else {
        $current_cat_id = 0;
    }
    ...
    $categories_query = "
        SELECT c.categories_id, cd.categories_name, c.categories_image, c.parent_id, 'category' as item_type
        FROM " . TABLE_CATEGORIES . " c
        LEFT JOIN " . TABLE_CATEGORIES_DESCRIPTION . " cd ON c.categories_id = cd.categories_id 
            AND cd.language_id = " . (int)$_SESSION['languages_id'] . "
        WHERE c.parent_id = " . (int)$current_cat_id . "
    ...
    if($categories->RecordCount() > 0) {
        while (!$categories->EOF) {
            ...
            $list_box_contents[] = $category;
            $categories->MoveNext();
        }
    }

    $products_query = "
        SELECT DISTINCT p.*, pd.products_name, 'product' as item_type
        FROM " . TABLE_PRODUCTS . " p
        LEFT JOIN " . TABLE_PRODUCTS_TO_CATEGORIES . " ptc ON p.products_id = ptc.products_id
        LEFT JOIN " . TABLE_PRODUCTS_DESCRIPTION . " pd ON p.products_id = pd.products_id 
            AND pd.language_id = " . (int)$_SESSION['languages_id'] . "
        WHERE (p.master_categories_id = " . (int)$current_cat_id . " 
            OR ptc.categories_id = " . (int)$current_cat_id . ")
    ...
    if($products->RecordCount() > 0) {
        while (!$products->EOF) {
            $product = array(
                ...
                'products_name' => $products->fields['products_name'] ?? 'Unknown Product',
                'products_price' => $products->fields['products_price'],
                'products_image' => $products->fields['products_image'],
                'products_date_added' => $products->fields['products_date_added'] ?? null,
            );
	...
	if (is_array($list_box_contents) && sizeof($list_box_contents) > 0) {
        ...
        foreach ($paged_list as $item) {
			if ($item['item_type'] == 'category') {
                // Display subcategory
                $category_path = zen_get_path($item['categories_id']);
                ?>
                <div class="watch-card watch-card-category">
                    <a href="<?php echo zen_href_link(FILENAME_DEFAULT, $category_path); ?>">
                        <div class="watch-card--image">
                            <?php if (!empty($item['categories_image'])) { ?>
                                <img src="<?php echo DIR_WS_IMAGES . $item['categories_image']; ?>" alt="<?php echo $item['categories_name']; ?>" />
                            <?php } else { ?>
                                <img src="<?php echo DIR_WS_IMAGES; ?>pixel_trans.gif" alt="<?php echo $item['categories_name']; ?>" />
                            <?php } ?>
                        </div>
                        <h3 class="watch-card--title"><?php echo $item['categories_name']; ?></h3>
                    </a>
                </div>
                <?php
            } else {
                // Display product (your existing product code)
                ?>
			<div class="watch-card watch-card-listing">
				...
				$creation_date = isset($item['products_date_added']) && $item['products_date_added'] ? 
					strtotime($item['products_date_added']) : 0;
				$is_new_product = ($creation_date > (time() - 604800));
				$query = "SELECT 
				...
				if (!$result->EOF) {
					...
					$newImageName = $fileParts['filename'] . ($ext ? "-01.$ext" : '');
	...
	<?php
        ...
        echo '<p>No products or categories found.</p>';
    }
    ?>
</div>
...
<div class="listing-pagination">
    <a href="/zencart-210-watxh/index.php?main_page=index&cPath=<?php echo $current_cat_id; ?>&page=1" class="prev" data-step="-1">
    ...
    for ($i = 1; $i <= $total_pages; $i++) {
        ...
        echo '<a ' . $active_class . ' href="/zencart-210-watxh/index.php?main_page=index&cPath='. $current_cat_id .'&page=' . $i . '">' . $i . '</a>';
    }
    ?>
    <a href="/zencart-210-watxh/index.php?main_page=index&cPath=<?php echo $current_cat_id; ?>&page=<?php echo $total_pages; ?>" class="next" data-step="1">
        <svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 11 11" fill="none">
