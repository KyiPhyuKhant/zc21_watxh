...
<?php
  $_SESSION['cowoa'] = isset($_SESSION['cowoa']) ? $_SESSION['cowoa'] : false;
  $cowoa_status = isset($_GET['cowoa']) ? $_GET['cowoa'] : '';
?>

<?php if (!isset($_SESSION['cowoa']) || !$_SESSION['cowoa']) { 
    if (zen_count_customer_address_book_entries() > 1) { 
?><h2 class="header-notactive"><?php echo TITLE_PLEASE_SELECT; ?></h2><?php 
    }
} ?>
  ...
  <div id="addressFields">
    ...
    <?php 
      if (defined('ENTRY_TITLE')) {
          echo zen_draw_input_field('address_title', '', zen_set_field_length(TABLE_ADDRESS_BOOK, 'entry_title', '40') . ' id="address_title"');
      } else {
          echo zen_draw_input_field('address_title', '', 'size="40" maxlength="40" id="address_title"');
      }
    ?>
    <br class="clearBoth" />
