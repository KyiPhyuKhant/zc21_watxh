        ...
        if (isset($quotes)) {
            for ($i = 0, $n = count($quotes); $i < $n; $i++) {
                ...
                <div class="shipping-method cf">
                    ...
                    <?php
                    if (OPRC_SHIPPING_INFO == 'true' && isset($quotes[$i]['info']) && $quotes[$i]['info'] != '') {
                        if (OPRC_SHOW_SHIPPING_METHOD_GROUP != 'true') {
                            echo '<h4 class="is-clickable">Shipping Info</h4>';
                        }
                    ...
                    if (isset($quotes[$i]['error'])) {
                        ...
                        $m_count = isset($quotes[$i]['methods']) ? count($quotes[$i]['methods']) : 0;
                        for ($j = 0, $n2 = $m_count; $j < $n2; $j++) {
                            // set the radio button to be checked if it is the method chosen
                            if (isset($_SESSION['shipping']['id']) && $quotes[$i]['id'] . '_' . $quotes[$i]['methods'][$j]['id'] == $_SESSION['shipping']['id']) {
                                $checked = true;
                            } else {
                                $checked = false;
                            }
