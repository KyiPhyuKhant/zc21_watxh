<?php
  ...
  $zencart_com_plugin_id = 1851; // from zencart.com plugins - Leave Zero not to check

  if($google_plus_one_button_version != 'new') {
    $config = $db->Execute("SELECT configuration_group_id FROM " . TABLE_CONFIGURATION . " WHERE configuration_key= 'GOOGLE_PLUS_ONE_VERSION'");
  ...
  while ($google_plus_one_button_version != $current_version) {
    switch($google_plus_one_button_version) {
      ...
      case '1.1.2':
        // perform upgrade
        if (file_exists(DIR_WS_INCLUDES . 'installers/google_plus_one_button/1_1_3.php')) {
          include_once(DIR_WS_INCLUDES . 'installers/google_plus_one_button/1_1_3.php');
          $google_plus_one_button_version = '1.1.3';          
		  break;
        } else {
          break 2;
		}          
      case '1.1.3':
        // perform upgrade
        if (file_exists(DIR_WS_INCLUDES . 'installers/google_plus_one_button/1_1_4.php')) {
          include_once(DIR_WS_INCLUDES . 'installers/google_plus_one_button/1_1_4.php');
          $google_plus_one_button_version = '1.1.4';          
      break;
        } else {
          break 2;
    }
